...
dc_pos = data['Positives'].dropna().diff().rolling('7d').sum().divide(data['Tested'].dropna().diff().rolling('7d',min_periods=5).sum())
...
fig.add_trace(go.Bar(
    x=bar_display['Positives'].dropna().index,
    y=bar_display['Positives'].dropna().diff(),
    name='New Cases',
...
fig = go.Figure(layout=layout)
fig.add_trace(go.Bar(
    x=bar_display['Deaths'].dropna().diff().index,
    y=bar_display['Deaths'].dropna().diff(),
    name='New Deaths',
    marker_color='maroon'
))
fig.add_trace(go.Scatter(
...
fig = go.Figure(layout=layout)
fig.add_trace(go.Bar(
    x=bar_display['Tested'].dropna().diff().index,
    y=bar_display['Tested'].dropna().diff(),
    name='New Tests',
    marker_color='orange'
    )
)
...
fig.add_trace(go.Scatter(
    ...
    y = data['Positives'].dropna().diff().divide(data['Tested'].dropna().diff()),
    mode='markers',
    marker=dict(
        color = 'orange'
    ),
    name = 'Single-Day',
    hovertemplate='New Tests: '+ data['Tested'].dropna().diff().map('{:,.0f}'.format)+
                  '<br>New Cases: '+ data['Positives'].dropna().diff().map('{:.0f}'.format)+
                  '<br>%{y:.1%} Positive'
))
fig.add_trace(go.Scatter(
    x = dc_pos.index,
...
#         legendgroup='Total Doses Delivered',
        hovertemplate='%{y:,.0f} Delivered<br>(%{text:.1%} Administered<extra>All Doses</extra>',
        text = vax_cum.sum(axis=1).divide(vax['Doses Delivered: District Allocation']+vax['Doses Delivered: Federal Partnership']).dropna(),
        mode='lines',
...
#         legendgroup='District Allocation',
        ...
        hovertemplate='%{y:,.0f} Delivered<br>(%{text:.1%} Administered',
        text = vax['Doses Administered: District Allocation'].divide(vax['Doses Delivered: District Allocation']).dropna(),
        showlegend=False,
    ),
    row=1,
    col=3
...
#         legendgroup='District Allocation',
        ...
        hovertemplate='%{y:,.0f} Delivered<br>(%{text:.1%} Administered',
        text = vax['Doses Administered: Federal Partnership'].divide(vax['Doses Delivered: Federal Partnership']).dropna(),
        showlegend=False,
    ),
    row=2,
    col=3
...
dc_cdc = pd.read_csv('https://data.cdc.gov/resource/8xkx-amqh.csv?fips=11001',index_col=0)
...
fig.add_trace(go.Scatter(
    x = dc_cdc.index,
    y = dc_cdc['administered_dose1_recip_18plus']/ward_demos.loc['All Wards','18+ (2019 ACS)'],
    name = '18+ (Partially)',
    line=dict(
        color = 'orange',
        dash='dot'
...
fig.add_trace(go.Scatter(
    x = dc_cdc.index,
    y = dc_cdc['administered_dose1_recip_65plus']/ward_demos.loc['All Wards','65+ (2019 ACS)'],
    name = '65+ (Partially)',
    line=dict(
        color = '#DD4477',
        dash='dot'
...
fig.add_trace(go.Scatter(
    x = dc_cdc.index,
    y = dc_cdc['series_complete_18plus']/ward_demos.loc['All Wards','18+ (2019 ACS)'],
    name = '18+ (Fully)',
    line=dict(
        color = 'orange',
    )
...
fig.add_trace(go.Scatter(
    x = dc_cdc.index,
    y = dc_cdc['series_complete_65plus']/ward_demos.loc['All Wards','65+ (2019 ACS)'],
    name = '65+ (Fully)',
    line=dict(
        color = '#DD4477',
    )
...
fig.update_layout(
    ...
    title=dict(
        text='Cumulative Vaccinations<br> of DC Residents (CDC Data)'
    )
)
